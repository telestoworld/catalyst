...
export class ServiceImpl implements MetaverseContentService, ClusterDeploymentsService {
  ...
  private async deployInternal(
    ...
    // Validate that telestoworld performed the deployment (only for legacy entities)
    validation.validatetelestoworldAddress(ownerAddress, validationContext)
  ...
  reportErrorDuringSync(
    ...
    return this.failedDeploymentsManager.reportFailure(
