...
export class ValidationContext {
  ...
  static readonly SYNCED: ValidationContext = ValidationContext.LOCAL.without(
    Validation.NO_NEWER,
    Validation.RECENT,
    Validation.REQUEST_SIZE,
    Validation.NO_REDEPLOYS
  )
  static readonly SYNCED_LEGACY_ENTITY: ValidationContext = ValidationContext.LOCAL_LEGACY_ENTITY.without(
    Validation.NO_NEWER,
    Validation.RECENT,
    Validation.NO_REDEPLOYS
  )
  static readonly OVERWRITTEN: ValidationContext = ValidationContext.SYNCED.without(Validation.CONTENT)
  static readonly OVERWRITTEN_LEGACY_ENTITY: ValidationContext = ValidationContext.SYNCED_LEGACY_ENTITY.without(
    Validation.CONTENT
  )
  static readonly FIX_ATTEMPT: ValidationContext = ValidationContext.ALL.without(
    Validation.REQUEST_SIZE,
    Validation.NO_NEWER,
    Validation.RECENT,
    Validation.telestoworld_ADDRESS,
  ...
  private without(...toRemove: Validation[]) {
    return new ValidationContext(Array.from(this.toExecute).filter((validation) => !toRemove.includes(validation)))
  }
}
